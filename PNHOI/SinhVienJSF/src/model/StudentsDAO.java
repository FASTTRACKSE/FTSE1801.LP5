...
public class StudentsDAO {	
	...
	public ArrayList<Students> getAllSinhVien(){
		...
		try {
			...
			while(result.next()) {
				...
				sinhVien.setId(result.getString("image"));
	...
	public boolean addNewSinhVien(Students sinhvien) {
		...
		String sql = "INSERT INTO `sinhvien`(`id`, `name`, `age`, `address`, `image`) VALUES (?,?,?,?,?)";
		try {
			...
			statement.setString(5, sinhvien.getImageName());
	...
	public ArrayList<Students> getAll(int trang,int soDong){
		...
		try {
			...
			while(result.next()) {
				...
				sv.setImageName(result.getString("image"));
