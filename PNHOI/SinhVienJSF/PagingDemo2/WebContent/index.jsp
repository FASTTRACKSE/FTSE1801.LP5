			...
			<c:when test="${dong<=soTrang }">
				<tr>
					<c:if test="${(dong > 2)&& (soTrang>3)}">
						<td><a href="PagingServlet?page=1">First</a></td>
					</c:if>
					<c:if test="${(dong ne  '1')&& (soTrang>2)}">
						<td><a href="PagingServlet?page=${dong - 1}">Previous</a></td>
					</c:if>
				    ...
				    <c:if test="${(dong ne soTrang) && (soTrang>2)}">
				    	<td><a href="PagingServlet?page=${dong+1}">Next</a></td>
				    </c:if>
				    <c:if test="${(dong < soTrang-1) && (soTrang>3)}">
						<td><a href="PagingServlet?page=${soTrang}">Last</a></td>
				    </c:if>
				 </tr>
			</c:when>
			<c:otherwise>
