				...
				<form:form method="POST" modelAttribute="taodonmoi">
					<div class="row">
						...
						<div>
							...
							<form:select multiple="single" path="maNhanVien.idNhanVien.maNhanVien"
								class="custom-select block round">
								<c:forEach items="${hoso}" var="nv">
									<form:option value="${nv.maNhanVien}"
										label="${nv.hoDem} ${nv.ten}" />
								</c:forEach>
							</form:select>
						</div>
						<br>
						<div>
							...
							<form:select multiple="single" path="idPhongBan.maPhongBan"
								class="custom-select block round">
						...
						<div>
							<label>Lý Do</label>
							<form:select path="idLoaiNghi.maLoaiNghi" class="custom-select block round"
								id="customSelect">
								<c:forEach items="${lydo}" var="ld">
									<form:option value="${ld.maLoaiNghi}" label="${ld.tenNgayNghi}" />
								</c:forEach>
							</form:select>
						</div>
						<br>
					...
					<div class="row">
						<div class="col-md-2">
							<button type="submit"
								...
								formaction="<%=request.getContextPath()%>/QuanLyVangNghi/taodonmoi/nhap">
								Lưu Nháp
							</button>
						</div>
