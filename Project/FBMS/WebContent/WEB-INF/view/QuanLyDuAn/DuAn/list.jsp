		...
		<div class="content-header-right col-md-3 col-xs-12">
			<div role="group" aria-label="Button group with nested dropdown"
				...
				<a href="/ffse-fbms/qlda/DuAn/add_form" class="btn btn-primary"><span
					class="fa fa-plus"></span>Thêm Dự Án</a>
			</div>
		</div>
						...
						<div class="card-body collapse in">
							<div class="card-block card-dashboard">
										...
										<div class="form-group col-sm-16">
											...
											<div class="form-group col-sm-3">
												<label>Khách Hàng</label> <select
													class="custom-select block round" name="maContact" id="maContact">
													<option value="0" label="Tất cả" />
													<c:forEach items="${contact}" var="ld">
														<option value="${ld.maContact}" label="${ld.tenContact} " />
													</c:forEach>
												</select>
											</div>
											<div class="form-group col-sm-3">
												<label>Tên Phòng Ban</label> <select
								...
								<%=request.getParameter("maDuAn")%>
									...
									maContact =
								<%=request.getParameter("maContact")%>
									;
									if (maContact != 0 && maContact != null) {
										$("#maContact").val(maContact);
									}
									maPhongBan =
									...
									<tbody>
										<c:forEach var="entities" items="${list }">
											<tr>
												...
												<td>${entities.contact.tenContact}</td>
		...
		<nav aria-label="Page navigation example">
			<ul class="pagination">
				<li class="page-item"><a class="page-link"
					href="/ffse-fbms/qlda/DuAn/list/1" <%=query%>>aa</a></li>
