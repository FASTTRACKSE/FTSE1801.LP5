package fasttrackse.ftse1801.fbms.dao.quantridanhgia;
...
public class QuanTriDanhGiaDaoImpl implements QuanTriDanhGiaDao{
	...
	public List<HoSoNhanSu> getListNhanVienPhongBan(PhongBan phongBan) {
		...
		if (session.createQuery("FROM HoSoNhanSu WHERE phongBan = :PhongBan AND chucDanh.maChucDanh = 'NV'")
				.setParameter("PhongBan", phongBan).list().size() > 0) {
			return session.createQuery("FROM HoSoNhanSu WHERE phongBan = :PhongBan AND chucDanh.maChucDanh = 'NV'")
