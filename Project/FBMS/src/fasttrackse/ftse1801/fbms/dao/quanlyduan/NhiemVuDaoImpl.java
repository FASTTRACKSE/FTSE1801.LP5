...
public class NhiemVuDaoImpl implements NhiemVuDao {
	...
	public List<NhiemVu> getByDuAn(int maDuAn, String search, int start, int maxRows) {
		Session session = this.sessionFactory.getCurrentSession();
		return session.createQuery("from NhiemVu where isDelete =1 and duAn.maDuAn='" + maDuAn + "' " + search, NhiemVu.class)
	...
	public int countNhiemvu(int maDuAn, String search) {
		Session session = sessionFactory.getCurrentSession();
		List<NhiemVu> nv = session.createQuery("from NhiemVu where isDelete = 1 and duAn.maDuAn='" + maDuAn + "' " + search, NhiemVu.class).list();
		return nv.size();
	}
	...
	public int checkVaiTro(int maNhanVien, String maVaiTro, int maDuAn) {
		Session session = sessionFactory.getCurrentSession();
		List<NhiemVu> nv = session.createQuery("from NhiemVu where duAn.maDuAn = '" + maDuAn + "' and vaiTroThanhVien.maVaiTro = '"
						+ maVaiTro + "' and hoSoNhanSu.maNhanVien= '" + maNhanVien + "' ", NhiemVu.class).list();
		return nv.size();
	}
}
