package fasttrackse.ftse1801.fbms.controller.quantridanhgia;
...
import fasttrackse.ftse1801.fbms.service.quantridanhgia.QuanTriDanhGiaService;
import fasttrackse.ftse1801.fbms.service.security.UserAccountService;
...
public class QuanTriDanhGiaController {
	...
	public HoSoNhanSu getInfo(HttpSession httpSession) {
	...
	private String loadHome(Model model, HttpSession httpSession) {
		HoSoNhanSu hoSo = getInfo(httpSession);
	...
	private String getDanhGiaBanThan(Model model, HttpSession httpSession) {
		HoSoNhanSu hoSo = gettInfo(httpSession);
		KyDanhGia kyDanhGia = service.getKyDanhGiaActive();
		model.addAttribute("danhGia", service.getDanhGiaBanThan(kyDanhGia, hoSo));
	...
	public String addDanhGiaBanThan(Model model, HttpSession httpSession) {
		HoSoNhanSu nhanVien = gettInfo(httpSession);
		KyDanhGia kyDanhGia = service.getKyDanhGiaActive();
		DanhGiaBanThan danhGia = new DanhGiaBanThan();
		danhGia.setKyDanhGia(kyDanhGia);
		danhGia.setPhongBan(nhanVien.getMaPhongBan());
	...
	public String createPhanCongDanhGia(Model model, @RequestParam("phongBan") String maPhongBan) {
		...
		if (service.checkPhanCongDanhGiaExist(kyDanhGia, phongBan) > 0) {
			...
			if (service.getListNhanVienPhongBan(phongBan).size() < 1) {
				...
				List<HoSoNhanSu> listNhanVien = service.getListNhanVienPhongBan(phongBan);
				List<DanhGiaNhanVien> danhGiaNhanVien = new ArrayList<>();
				for (HoSoNhanSu x : listNhanVien) {
					List<HoSoNhanSu> listNhanVienLimit = service.getNhanVienLimit(x.getMaNhanVien(), phongBan);
					for (HoSoNhanSu y : listNhanVienLimit) {
	...
	public String getListDanhGiaBanThan(Model model, @PathVariable int page, HttpSession session) {
		HoSoNhanSu nhanVien = gettInfo(session);
		KyDanhGia kyDanhGia = service.getKyDanhGiaActive();
		int start = (page - 1) * maxItems;
		model.addAttribute("start", start);
		session.setAttribute("pageDDG", page);
		model.addAttribute("total", Math.ceil(
				(double) service.getListDanhGiaBanThanPhongBan(kyDanhGia, nhanVien.getMaPhongBan()) / (double) maxItems));
		model.addAttribute("listNhanVien",
				service.getListDanhGiaBanThanPhongBan(start, maxItems, kyDanhGia, nhanVien.getMaPhongBan()));
	...
	public String getListDanhGiaNhanVien(@PathVariable int page, Model model, HttpSession session) {
		HoSoNhanSu nhanVien = gettInfo(session);
		KyDanhGia kyDanhGia = service.getKyDanhGiaActive();
		if (service.getListNhanVienPhongBan(nhanVien.getMaPhongBan()).size() < 1) {
			createListDanhGiaNhanVien(kyDanhGia, nhanVien.getMaPhongBan());
		...
		model.addAttribute("start", start);
		model.addAttribute("total", Math.ceil(
				(double) service.getListTruongPhongDanhGia(kyDanhGia, nhanVien.getMaPhongBan()) / (double) maxItems));
		model.addAttribute("listDanhGia",
				service.getListTruongPhongDanhGia(start, maxItems, kyDanhGia, nhanVien.getMaPhongBan()));
