...
<body>
	...
	<div class="dropdown" style="text-align: left;">
		<button class="btn btn-primary dropdown-toggle" stype="button"
			data-toggle="dropdown">
			Language <span class="caret"></span>
		</button>
		<ul class="dropdown-menu">
			<li><a href="KhachHangServlet?language=vi&page=${page}">VietNam</a></li>
			<li><a href="KhachHangServlet?language=en&page=${page}">English</a></li>
		</ul>
	</div>
	<form action="AddKH" method="get">
		...
		<input type="submit" value="<fmt:message key="them"/>" name="luaChon"
			class="nhap">
	</form>
	<table>
		<tr>
			...
			<c:choose>
				<c:when test="${i%2 eq 0}">
					<tr class="a">
						...
						<td><a href="DeleteKh?MaKH=${khachHang.maKH}"><fmt:message
									key="xoa" /></a></td>
						<td><a href="UpdateKh?MaKH=${khachHang.maKH}"><fmt:message
									key="sua" /></a></td>
					</tr>
					<c:set var="i" value="${i+1}"></c:set>
				</c:when>
				<c:otherwise>
					<tr class="b">
						...
						<td>${khachHang.soTienTrongTK}</td>
						<td><a
							href="DeleteKh?MaKH=${khachHang.maKH}$language=${lang}"><fmt:message
									key="xoa" /></a></td>
						<td><a
							href="UpdateKh?MaKH=${khachHang.maKH}&language=${lang}"><fmt:message
									key="sua" /></a></td>
					</tr>
					<c:set var="i" value="${i+1}"></c:set>
				</c:otherwise>
	...
	<form action="KhachHangServlet" method="get">
		<ul class="pagination">
			<c:if test="${page ne 1 && soTrang > 2}">
				<li><a href="KhachHangServlet?page=1&PageEnd=3&PageBegin=1"
					class="link"> <c:out value="<<"></c:out>
				</a></li>
			</c:if>
			<c:if test="${page ne 1}">
				<li><a href="KhachHangServlet?page=1" class="link"> <c:out
							value="<"></c:out>
				</a></li>
			</c:if>
			<c:choose>
				...
				<c:when test="${soTrang eq 2}">
					<li><a href="KhachHangServlet?page=1&PageEnd=2&PageBegin=1"
						class="link">1</a></li>
					<li><a href="KhachHangServlet?page=2&PageEnd=2&PageBegin=1"
						class="link">2</a></li>
				</c:when>
				<c:otherwise>
					<c:choose>
						<c:when test="${page eq 1}">
							<c:forEach var="i" begin="1" end="3">
								<li><a
									href="KhachHangServlet?page=${i}&PageEnd=3&PageBegin=1"
									class="link">${i}</a></li>
							</c:forEach>
						</c:when>
						<c:when test="${page eq soTrang}">
							<c:forEach var="i" begin="${soTrang-2}" end="${soTrang}">
								<li><a
									href="KhachHangServlet?page=${i}&PageEnd=${soTrang}&PageBegin=${sotrang-2}"
									class="link">${i}</a></li>
							</c:forEach>
						</c:when>
						<c:when test="${page eq pageBegin}">
							<c:forEach var="i" begin="${page-1}" end="${page+1}">
								<li><a
									href="KhachHangServlet?page=${i}&PageEnd=${page+1}&PageBegin=${page-1}"
									class="link">${i}</a></li>
							</c:forEach>
						</c:when>
			...
			<c:if test="${page ne soTrang}">
				<li><a
					href="KhachHangServlet?page=${page+1}&PageEnd=${page+3}&PageBegin=${page}"
					class="link">></a></li>
			</c:if>
			<c:if test="${page ne soTrang && soTrang>2}">
				<li><a
					href="KhachHangServlet?page=${soTrang}&PageEnd=${soTrang}&PageBegin=${soTrang-2}"
					class="link">>></a></li>
			</c:if>
		</ul>
	</form>
	<form action="DangSuat" method="post">
		<input type="submit" value='<fmt:message key="DangXuat"/>'>
	</form>
